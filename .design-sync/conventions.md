...
| Bausteine | `wp-chip` (Auswahl, 8px), `wp-img` (Bild, 12px), `wp-cta` (einziger Button), `wp-link`, `wp-role` (Fachgebiet über dem Namen), `wp-card` (Linie statt Schatten), `wp-sum` + `wp-foot` (große Summe auf Petrol), `wp-logo` + `wp-w1`/`wp-w2`, `wp-icon`, `wp-badge` |
