...
- **Baukasten-Konfigurator:** neun Handlungsfelder auf 3×3-Grundplatte, zwei Gruppen. Energie: Strom, Wärme, Kälte, Druckluft, Speicher. Ressourcen: Material, Wasser, Reststoffe, Betriebsstoffe. **Kein PV**, keine Gruppe „Eigenerzeugung“. Steine abgerundet, oben Medien-Symbol, vorne Prozessname (passt sich der Breite an). **Keine Auswahl mehr** (seit Version 89, „Warum kann man hier rumklicken“): alle neun Steine stehen fest auf der Platte. Stattdessen („Mach etwas damit“, Version 90): Die Steine bauen sich beim Hereinscrollen nacheinander auf (`#plate.on`, `.blk.stone` mit `rise`), und die Legende darunter (`button.chip`) hebt beim Überfahren/Tippen den zugehörigen Stein hervor (andere werden gedimmt) und zeigt darunter eine Zeile aus `NOTE` (`#bk-note`, ruhig, Sans, kein Kalam). Dazu (seit Version 76, Wunsch „alle 6 Punkte umsetzen“) ein Satz zum Ablauf: „Für jede Maßnahme: messen, nachrechnen, umsetzen. Sie entscheiden, welche wir angehen.“ (Wunsch: „Maßnahme“ statt Stein/Platte.) Keine Erklär-Box, keine Zahlenzeile.
